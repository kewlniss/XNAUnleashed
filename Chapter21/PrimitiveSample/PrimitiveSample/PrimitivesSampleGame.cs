...
namespace PrimitiveSample
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        private void DrawShip(Vector2 where)
        {
            ...
            primitiveBatch.AddVertex(
                where + new Vector2(-ShipSizeX, ShipSizeY), Color.Red);

            // to the right and up, into the cutout
            primitiveBatch.AddVertex(
                where + new Vector2(-ShipSizeX, ShipSizeY), Color.Blue);
            primitiveBatch.AddVertex(
                where + new Vector2(0f, ShipSizeY - ShipCutoutSize), Color.Green);

            // to the right and down, out of the cutout
            primitiveBatch.AddVertex(
                where + new Vector2(0f, ShipSizeY - ShipCutoutSize), Color.Yellow);
            primitiveBatch.AddVertex(
                where + new Vector2(ShipSizeX, ShipSizeY), Color.Purple);

            // and back up to the nose, where we started.
            primitiveBatch.AddVertex(
                where + new Vector2(ShipSizeX, ShipSizeY), Color.Cyan);
            primitiveBatch.AddVertex(
                where + new Vector2(0f, -ShipSizeY), Color.Orange);
